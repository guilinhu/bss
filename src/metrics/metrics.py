...
class Metrics(nn.Module):
    def __init__(self, name, fs=16000, **kwargs) -> None:
        ...
        if name == "snr":
            ...
            self.func = lambda est, gt, mix: PESQ(
                preds=resample_fn(est, fs, fs_new), target=resample_fn(gt, fs, fs_new), fs=fs_new, mode="wb"
            )
        elif name == "NBPESQ":
            fs_new = 8000
            resample_fn = lambda x, y, z: x
            if fs_new != fs:
                resample_fn = resample
            self.func = lambda est, gt, mix: PESQ(
                preds=resample_fn(est, fs, fs_new), target=resample_fn(gt, fs, fs_new), fs=fs_new, mode="nb"
            )
    ...
    def forward(self, est, gt, mix=None):
        ...
        # print("est is {}".format(est.shape))
        # print("gt is {}".format(gt.shape))
